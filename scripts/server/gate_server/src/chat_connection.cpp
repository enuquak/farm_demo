...
bool ChatConnection::connect(const std::string& ip, uint16_t port) {
    ...
    if (fd < 0) {
        SPDLOG_ERROR("[ChatConnection]Failed to create socket");
        return false;
    }
    ...
    if (!bev_) {
        SPDLOG_ERROR("[ChatConnection]Failed to create bufferevent");
    ...
    if (bufferevent_socket_connect(bev_,
        ...
        SPDLOG_ERROR("[ChatConnection]Failed to connect to {}:{}", ip, port);
        disconnect();
        return false;
    }

    SPDLOG_INFO("[ChatConnection]Connecting to {}:{}", ip, port);
    return true;
}
...
bool ChatConnection::send(uint32_t msg_id, const uint8_t* payload, size_t len) {
    if (!bev_ || state_ == ChatConnState::DISCONNECTED) {
        return false;
    }
...
void ChatConnection::start_reconnect() {
    ...
    SPDLOG_INFO("[ChatConnection]Reconnect timer started (5s interval)");
}
...
void ChatConnection::handle_connect_success() {
    SPDLOG_INFO("[ChatConnection]Connected to {}:{}", chat_ip_, chat_port_);
...
void ChatConnection::handle_disconnect() {
    SPDLOG_INFO("[ChatConnection]Disconnected from {}:{}", chat_ip_, chat_port_);
...
void ChatConnection::send_identify() {
    ...
    SPDLOG_INFO("[ChatConnection]Sent GATE_IDENTIFY (gate_id={})", gate_id_);
}

void ChatConnection::send_heartbeat() {
    if (state_ != ChatConnState::IDENTIFIED) return;
...
void ChatConnection::try_reconnect() {
    if (state_ != ChatConnState::DISCONNECTED) return;

    SPDLOG_INFO("[ChatConnection]Trying to reconnect to {}:{}", chat_ip_, chat_port_);
